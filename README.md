...
This repository contains an implementation of the work sample problem specified
below.

## Prerequisites

- Ruby 2.5.3 (earlier or later versions will likely work ok but this was the
  version I used when testing)
- If running on Windows, you may also need to install
  [tzinfo-data](https://github.com/tzinfo/tzinfo/wiki/Resolving-TZInfo::DataSourceNotFound-Errors)

## How to

```bash
# Fetch the code
git clone https://github.com/mxhold/truss_csv_normalizer.git
cd truss_csv_normalizer/
...
# Run
cat spec/input.csv | bin/normalize_csv

# can also provide input via file:
bin/normalize_csv spec/input.csv
```

## Assumptions

Since it's not specified in the original instructions, I'm assuming:

- 2 digit years between "00" and "68" should be expanded to the years between
  2000 and 2068
- 2 digit years between "69" and "99" should be expanded to the years between
  1969 and 1999
- Capitalizing "i" as "I" instead of "İ" (LATIN CAPITAL LETTER I WITH DOT
  ABOVE) is ok since I have no way of knowing if a name is Turkish
